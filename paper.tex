...
\label{sec:automotive}
	...
	Therefore, to minimize the end-to-end latency, we need to minimize the maximal length of both $\pc^p_{W_p}$ and $\pc^p_{W_p+1}$.
	However, Figure~\ref{fig:automotive_example} demonstrates that it is not possible to eliminate the slack between write- and read-events of subsequent jobs of both $\pc^p_{W_p}$ and $\pc^p_{W_p+1}$.
	%
	\begin{figure}
		\centering
		\subfloat[Minimize $\ell(\pc^p_{W_p})$.]{
		...
		\\
		\subfloat[Minimize $\ell(\pc^p_{W_p+1})$.]{
			\begin{tikzpicture}[yscale=0.25, xscale=0.27]
			\begin{scope}[shift={(0,6)}] % task one
				\taskname{$\tau_1$}
		...
		\caption{Example of three tasks demonstrating that not both $\ell(\pc^p_{W_p})$ and $\ell(\pc^p_{W_p+1})$ can be minimized simultaneously. The partitioned job chain $\pc^p_{W_p}$ is marked in red and $\pc^p_{W_p+1}$ is marked in blue. The jobs belonging to both are half red and half blue. Gaps are depicted as left-right arrows.}
		\label{fig:automotive_example}
	\end{figure}
	%
	The example consists of a cause-effect chain $E = (\tau_1 \to \tau_2 \to \tau_3 )$ with three tasks.
	The tasks have periods $T_{\tau_1} = 5$, $T_{\tau_2} = 1$ and $T_{\tau_3} = 2$.
	Therefore, that cause-effect chains falls in case \ref{item:automotive_max_harm_case4}, and $\Tbb_E$ is not max-harmonic.
	If we minimize $\ell(\pc^p_{W_p})$, the write-event of $\tau_2$ and the read-event of $\tau_3$ do not align for $\pc^p_{W_p+1}$, as depicted in Figure~\ref{fig:automotive_example:optimal_first}.
	Conversely, if we minimize $\ell(\pc^p_{W_p+1})$, the write-event of $\tau_2$ and the read-event of $\tau_3$ do not align for $\pc^p_{W_p}$, as depicted in Figure~\ref{fig:automotive_example:optimal_second}.
