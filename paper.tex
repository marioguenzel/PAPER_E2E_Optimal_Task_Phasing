...
\label{sec:harmonic}
	...
	Naturally, if $\Tbb$ is harmonic, then $\Tbb$ and all its subsets $\Tbb' \subseteq \Tbb$ are also max-harmonic. 
	In particular, $\Tbb_E$ is max-harmonic.
	
	To develop an optimal task phasing strategy, we first formulate a general lower bound on the end-to-end latency. 
	...
	In the following, we develop a task phasing that ensures that the end-to-end latency reaches this lower bound for max-harmonic tasks. 
